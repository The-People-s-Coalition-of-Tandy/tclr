    ...
    for (int i = 0; i < 200; ++i) {
        ...
        if (h.x < prob) {
            ...
            if (t < dist) {
                ...
                t = distance(center, p);
                fog *= smoothstep(1.0, 0.5, cos(tau * (beaconFreq * u_time + h.y)));
...
void main () {
    ...
    // camera controls
    vec2 m = vec2(0.03 * u_time, 1.4);
    m *= tau * vec2(1.0, 0.25);

    float increm = abs(sin(u_time/10.));
    float lerpZoom = mix(-15.,30.,increm);

    // last arg - zoom
    vec3 center = vec3(4.0 * u_time, 0.5, lerpZoom);
    float dist = 20.0;
